...
Lagarias criterion, harmonic numbers, colossally abundant numbers,
Nicolas criterion, formal verification, Lean}
...
We then formalize Lagarias's elementary criterion
\[
  \sig(n)\le H_n+e^{H_n}\log H_n\qquad(n\ge1)
\]
by reusing the completed Robin implications, proving Lagarias's two harmonic
comparison estimates, and checking the remaining range $2\le n\le5040$ with
exact kernel certificates.
...
\section{Lagarias's elementary criterion}\label{sec:lagarias}

Write $H_n=\sum_{k=1}^n 1/k$.  Lagarias proved the following elementary
reformulation of the Riemann hypothesis~\cite{Lagarias2002}.

\begin{theorem}[Lagarias]\label{thm:lagarias}
The Riemann hypothesis holds if and only if every positive integer $n$
satisfies
\begin{equation}\label{eq:lagarias}
  \sig(n)\le H_n+e^{H_n}\log H_n.
\end{equation}
\end{theorem}

The formal proof deliberately reuses the hard Robin work rather than
reconstructing it.  For the forward implication, Lagarias's Lemma~3.1 gives,
for $n\ge3$,
\begin{equation}\label{eq:lagarias-lower}
  e^{\euler}n\log\log n\le e^{H_n}\log H_n.
\end{equation}
Indeed $\euler+\log n\le H_n$, exponentiation gives
$e^{\euler}n\le e^{H_n}$, and
$\log n\le H_n$ gives $\log\log n\le\log H_n$; all factors have the
required signs.  Under $\RH$, Theorem~\ref{thm:main} and
\eqref{eq:lagarias-lower} prove \eqref{eq:lagarias} for $n>5040$.
The case $n=1$ is equality.  The interval $2\le n\le5040$ is split into
46 consecutive blocks.  Each block records an exact upper bound for
$\sig(n)$ throughout the block and rational lower bounds for $H_n$,
$e^{H_n}$, and $\log H_n$.  The reusable block soundness theorem converts
those checks into \eqref{eq:lagarias}; all arithmetic checks use
\path{decide +kernel}, not \path{native_decide}.

For the converse, a relaxed form of Lagarias's Lemma~3.2 suffices:
for $n\ge20$,
\begin{equation}\label{eq:lagarias-upper}
 H_n+e^{H_n}\log H_n
 \le e^{\euler}n\log\log n+\frac{24n}{\log n}.
\end{equation}
The published lemma has the sharper constant $7$; no sharp constant is
needed here.  Apply \eqref{eq:lagarias-upper} to
$L_P=\mathop{\rm lcm}(1,\ldots,P)$.  If \eqref{eq:lagarias} holds, the
positive logarithmic Robin margin at $L_P$ is at most
$24/\log L_P$.  The elementary Chebyshev lower bound
$\log L_P\gg P$ makes this $O(P^{-1})$.  The already formalized
Nicolas--Landau argument under $\neg\RH$ supplies positive margin excursions
of order $P^{-b}$ for some $0<b<1/2$, while $P^{-1}=o(P^{-b})$.
The two estimates contradict one another for arbitrarily large $P$.

The corresponding Lean declarations are
\path{robinBound_le_exp_harmonic_mul_log_harmonic},
\path{lagarias_upper_bound},
\path{lagarias_finite_two_to_5040}, and
\path{Robin1984.riemannHypothesis_iff_lagariasElementaryCriterion}, in
\codedir{Robin1984/Lagarias}.  The first and third parts encode Lagarias's
forward argument and bounded verification; the converse calls the existing
LCM oscillation theorem directly.
...
The preceding proofs have been formalized in Lean~4.33.1 using
Mathlib~\cite{Lean4,Mathlib}.  The three public equivalence declarations have the
mathematical types
\begin{align*}
 ...
 &\RH\ \Longleftrightarrow\
   ...
   \sig(n)<e^{\euler}n\log\log n),\\
 &\RH\ \Longleftrightarrow\
   (\forall n\ge1,\ \sig(n)\le H_n+e^{H_n}\log H_n).
\end{align*}
They are named
\begin{center}
\small
...
\path{Robin1984.riemann_hypothesis_iff_lagarias_elementary_criterion}.
\end{center}
All three statement boundaries use Mathlib primitives.  The corresponding
solution files supply the proof terms, so statement comparison is independent
of the internal proof graph.
...
Lagarias criterion &
  \codelink{Robin1984/Lagarias/Theorem.lean}{Lagarias.Theorem}
  combines the Robin forward theorem, a 46-block finite certificate, and the
  existing Nicolas--Landau converse \\
CA restriction &
  \codelink{Robin1984/ColossallyAbundant/CAReduction.lean}{CAReduction}
...
The two Robin biconditionals also have different quantifier surfaces
...
from those imported results to the three advertised biconditionals, including
the weighted endpoint, Nicolas--Landau continuation, LCM transfer, CA
reduction, Lagarias harmonic comparisons, and finite certificates, is proved
here without adding a new axiom.
...
narrow Mathlib modules.  It defines the divisor sum, Robin inequality, CA
predicate, and Lagarias criterion directly, and states the three advertised
theorems with deliberate proof holes.  The
\codelink{Solution.lean}{Solution} module imports the project and supplies the
actual proofs.  The checked
\codelink{comparator.json}{Comparator configuration} names exactly those three
...
\[
  ...
  \leanname{Quot.sound}.
\]
This audit follows the proof terms of the three advertised declarations through
...
Its default targets are \leanname{Challenge} and \leanname{Solution}; they use
Lake's ordinary scheduler, allowing unrelated modules to build concurrently.
The memory-intensive finite-certificate modules encode their order through
import dependencies,
while each Lean process is bounded to one internal task thread.  Thus the
project source controls the expensive certificate order without an external
runner or local patching step.
...
Amer. Math. Monthly 109 (2002), no. 6, 534--543,
\href{https://doi.org/10.1080/00029890.2002.11919883}
{doi:10.1080/00029890.2002.11919883};
preprint submitted in 2001,
\href{https://arxiv.org/abs/math/0008177}{arXiv:math/0008177}.
