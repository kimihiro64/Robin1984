...
\end{equation}
All quantities in \eqref{eq:margin} are then defined and positive where
needed, and Robin's inequality is equivalent to $\M(n)<0$.
...
$\log n=100000$.  The exact cutoff therefore requires a proof for all
...
\begin{proof}
Thirty-nine rational rows form a gap-free chain from $336/25$ to $100000$.
Since $336/25<\log720720$, they cover \eqref{eq:finite-log-range}.  Every row
satisfies the hypotheses of Lemma~\ref{lem:row-sound}.  In the final row the
first-layer product includes every prime below $95673$; it is split into 132
contiguous blocks, whose exact products concatenate to the required prefix.
...
If $5040<n$ and $\log n<100000$, then Robin's inequality holds.
\end{theorem}
...
$\log\log H$ plus Mertens's constant and a weighted $\vartheta$-error.
...
\]
At $H=100000$, rational enclosures for $c_0$, $\log H$, and the negative
powers make this expression strictly smaller than $9/4$; monotonicity keeps
the bound valid for larger $H$.
...
\begin{align*}
  ...
  &\quad-4\log4\,H^{-1/2}
     \frac{\log H}{\log H-\log2}.
\end{align*}
Exact rational bounds for $\sqrt2$, $2^{1/4}$, and the logarithmic ratios
show that this is at least $113/50$.
\end{proof}
...
\begin{proof}
If $\log n<100000$, use Theorem~\ref{thm:finite}.  Otherwise combine
Propositions~\ref{prop:budget} and~\ref{prop:coefficient-gap}:
\[
  \M(n)
  <\left(\frac94-\frac{113}{50}\right)S(\log n)
  =-\frac1{100}S(\log n)<0.
\]
...
\begin{proof}
Mertens's convergent summand series and partial summation give the exact
...
The preceding proof has been formalized in Lean~4.33.1 using Mathlib.  The two
public declarations have the mathematical types
\begin{align*}
...
factorizations and the 39 interval rows reduce to exact natural-number and
rational inequalities.  The large final prime product is split into 132
...
Its Lake-native default target reads a checked-in topological order of 252
modules, schedules one module job at a time, bounds each Lean process to two
internal task threads, and ends with \leanname{Solution}.  Thus the submitted
...
finite Mertens function are due to Jean-Louis Nicolas, and the positivity
...
Riemann},
J. Math. Pures Appl. (9) 63 (1984), 187--213, MR0774171.
