...
/-!
# Explicit complete second-weight tails above 400
...
theorem robin_log_ge_five {x : Real} (hx : 400 <= x) : 5 <= Real.log x := by
...
theorem robin_root_error_upper_log_five
    {n k : Nat} (hn : 1 <= n) (hk : 2 <= k) {x : Real} (hx : 400 <= x) :
...
theorem robin_root_error_two_normalized
    {k : Nat} (hk : 2 <= k) {x b : Real} (hx : 400 <= x)
...
theorem robin_second_weight_root_ratios {x : Real} (hx : 400 <= x) :
    And (x^(-(1 / 6 : Real)) <= 3 / 8)
      (And (x^(-(3 / 10 : Real)) <= 1 / 6)
        (And (x^(-(1 / 4 : Real)) <= 9 / 40)
          (And (x^(-(1 / 3 : Real)) <= 11 / 80)
            (x^(-(2 / 5 : Real)) <= 23 / 250)))) := by
  have hxPos : 0 < x := by linarith
  have hSix := robin_rpow_neg_div_le_of_pow (a := 1) (d := 6) (b := (3 / 8 : Real))
    hxPos (by norm_num) (by omega) (by norm_num; linarith)
  have hTen : x^(-(3 / 10 : Real)) <= 1 / 6 := by
    apply robin_rpow_neg_div_le_of_pow (a := 3) (d := 10) hxPos (by norm_num) (by omega)
    have hPow : (400 : Real)^3 <= x^3 := by gcongr
    norm_num at hPow
    norm_num
    linarith
  have hFour := robin_rpow_neg_div_le_of_pow (a := 1) (d := 4) (b := (9 / 40 : Real))
    hxPos (by norm_num) (by omega) (by norm_num; linarith)
  have hThree := robin_rpow_neg_div_le_of_pow (a := 1) (d := 3) (b := (11 / 80 : Real))
    hxPos (by norm_num) (by omega) (by norm_num; linarith)
  have hFive : x^(-(2 / 5 : Real)) <= 23 / 250 := by
    apply robin_rpow_neg_div_le_of_pow (a := 2) (d := 5) hxPos (by norm_num) (by omega)
    have hPow : (400 : Real)^2 <= x^2 := by gcongr
    norm_num at hPow
    norm_num
    nlinarith
...
theorem robinPrimePowerWeightedTail_two_upper
    ...
    robinPrimePowerWeightedTail 2 x <=
      (17 / 8 : Real) * (x^(-(3 / 2 : Real)) * Inv.inv (Real.log x)) := by
  have hxOne : 1 < x := by linarith
  have hLogPos := Real.log_pos hxOne
  ...
  have hTwoError := robin_root_error_two_normalized (k := 2) (by omega) hx
    (b := (9 / 40 : Real)) (by norm_num; exact hRatios.2.2.1)
  have hThreeError := robin_root_error_two_normalized (k := 3) (by omega) hx
    (b := (11 / 80 : Real)) (by norm_num; exact hRatios.2.2.2.1)
  have hFiveError := robin_root_error_two_normalized (k := 5) (by omega) hx
    (b := (23 / 250 : Real)) (by norm_num; exact hRatios.2.2.2.2)
...
theorem robinPsiWeightedErrorScalar_two_upper {x : Real} (hx : 400 <= x) :
    robinPsiWeightedErrorScalar 2 x <=
      (1 / 8 : Real) * (x^(-(3 / 2 : Real)) * Inv.inv (Real.log x)) := by
  have hxOne : 1 < x := by linarith
  have hLogPos := Real.log_pos hxOne
  have hq : Inv.inv (Real.log x) <= (1 / 5 : Real) := by
  ...
  calc
    _ <= (1 / 20 : Real) * (x^(-(3 / 2 : Real)) * Inv.inv (Real.log x)) *
      ...
      gcongr
      exact robin_zero_constant_le_one_twentieth
    _ <= (1 / 20 : Real) * (x^(-(3 / 2 : Real)) * Inv.inv (Real.log x)) * (5 / 2) := by gcongr
    _ = _ := by ring
