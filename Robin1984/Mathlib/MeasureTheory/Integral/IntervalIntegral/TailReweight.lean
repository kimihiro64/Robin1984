...
This file proves a Fubini identity that rewrites an integrable tail against an
increasing absolutely continuous weight while retaining its boundary term.
-/
...
/-- Triangular integrand used to exchange a complete tail with its weight. -/
def tailTriangle (v : Real → Real) (F : Real → Complex) (a t : Real) : Complex :=
  if a ≤ t then (v a : Complex) * F t else 0

theorem integral_tailTriangle_left
    {x t : Real} (v : Real → Real) (F : Real → Complex) :
    integral (volume.restrict (Ioi x)) (fun a => tailTriangle v F a t) =
      ((integral (volume.restrict (Ioc x t)) v : Real) : Complex) * F t := by
  have hFunction : (fun a => tailTriangle v F a t) =
      (Iic t).indicator (fun a : Real => (v a : Complex) * F t) := by
    funext a
    simp [tailTriangle, Set.indicator]
...
theorem integral_norm_tailTriangle_left
    {x t : Real} {v : Real → Real} (F : Real → Complex)
    (hv : ∀ a : Real, x < a → 0 ≤ v a) :
    integral (volume.restrict (Ioi x)) (fun a => ‖tailTriangle v F a t‖) =
      integral (volume.restrict (Ioc x t)) v * ‖F t‖ := by
  have hFunction : (fun a : Real => ‖tailTriangle v F a t‖) =
      (Iic t).indicator (fun a : Real => ‖(v a : Complex)‖ * ‖F t‖) := by
    funext a
    by_cases ha : a ≤ t <;> simp [tailTriangle, Set.indicator, ha]
  rw [hFunction, setIntegral_indicator measurableSet_Iic]
  have hSet : Set.inter (Ioi x) (Iic t) = Ioc x t := by
    ext a
    rfl
...
theorem integral_tailTriangle_right
    {x a : Real} (ha : x < a) (v : Real → Real) (F : Real → Complex) :
    integral (volume.restrict (Ioi x)) (fun t => tailTriangle v F a t) =
      (v a : Complex) * integral (volume.restrict (Ioi a)) F := by
  have hFunction : (fun t => tailTriangle v F a t) =
...
integrability assumptions retain both the actual tail and its boundary term. -/
theorem complete_tail_reweight
    {x : Real} {h v : Real → Real} {F : Real → Complex}
    (hvMeas : Measurable v) (hFMeas : Measurable F)
    (hv : ∀ a : Real, x < a → 0 ≤ v a)
    (hvInt : ∀ t : Real, x < t → IntegrableOn v (Ioc x t))
    (hPrimitive : ∀ t : Real, x < t →
      ...
      (integral (volume.restrict (Ioi x)) (fun t : Real => (h t : Complex) * F t) =
        (h x : Complex) * integral (volume.restrict (Ioi x)) F +
          integral (volume.restrict (Ioi x)) (fun a : Real =>
            (v a : Complex) * integral (volume.restrict (Ioi a)) F)) := by
  let J : Prod Real Real → Complex := fun z => tailTriangle v F z.1 z.2
  have hMeas : Measurable J := by
    unfold J tailTriangle
    apply Measurable.ite (measurableSet_le measurable_fst measurable_snd)
    · fun_prop
    · exact measurable_const
  have hLeftInt : ∀ t : Real, x < t →
      IntegrableOn (fun a : Real => tailTriangle v F a t) (Ioi x) := by
    intro t ht
    have hFunction : (fun a => tailTriangle v F a t) =
        (Iic t).indicator (fun a : Real => (v a : Complex) * F t) := by
      funext a
  ...
  have hNormInt : IntegrableOn (fun t : Real =>
      integral (volume.restrict (Ioi x)) (fun a : Real => ‖J (a, t)‖)) (Ioi x) := by
    ...
    rw [integral_norm_tailTriangle_left F hv, hPrimitive t ht]
  ...
  have hJoint : Integrable J
      ((volume.restrict (Ioi x)).prod (volume.restrict (Ioi x))) := by
  ...
  have hRightInt : IntegrableOn (fun a : Real =>
    ...
    intro a ha
    exact integral_tailTriangle_right ha v F
  have hSwap := integral_integral_swap (f := tailTriangle v F) hJoint
  have hLeft : integral (volume.restrict (Ioi x)) (fun t : Real =>
    ...
    calc
      _ = integral (volume.restrict (Ioi x)) (fun t : Real =>
        ...
        intro t ht
        dsimp only [J]
        rw [integral_tailTriangle_left, hPrimitive t ht, Complex.ofReal_sub, sub_mul]
      _ = _ := by
