...
import Submission.PrimeNumberTheoremAnd.IEANTN.RosserSchoenfeld.RosserSchoenfeldPrime_tables
...
theorem eq_413 {f : ℝ → ℝ} {x : ℝ} (hx : 2 ≤ x) (hf : ∀ t ∈ Set.Icc 2 x, DifferentiableAt ℝ f t)
  ...
  have ha (n : ℕ) : a n = if Nat.Prime n then Real.log n else 0 := by
  ...
  have hsum (y : ℝ) :
      (∑ n ∈ Icc 0 ⌊y⌋₊, if Nat.Prime n then Real.log n else 0) = θ y := by
    rw [theta_eq_sum_Icc, Finset.sum_filter]
